...
conf = vg_config()
conf.MODEL_TYPE = "EfficientNetB3"
...
for index, platform in enumerate(conf.platform_info):
    ...
    if platform == 143:
        ...
        for i in range(1,6):
            history = np.load(f'results/history_2023_11/vg-history-platform{platform}-{conf.MODEL_TYPE}-{i}.npy', allow_pickle=True).item()
            epochs[index,i-1] = len(history['loss'])
m_epochs = np.mean(epochs,axis=1)
s_epochs = np.std(epochs,axis=1)
np.savetxt('m_epochs.csv', m_epochs, fmt='%.2f', delimiter=', ', newline='\n')
np.savetxt('s_epochs.csv', s_epochs, fmt='%.2f', delimiter=', ', newline='\n')
