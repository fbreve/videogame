...
def load_data():
    gamelist = pd.read_csv("./data/gamelist-" + str(conf.PLATFORM) + ".csv", index_col=0)
    screenshotlist = pd.read_csv("./data/screenshotlist-" + str(conf.PLATFORM) + ".csv", index_col=0)
    screenshotcount = pd.read_csv("./data/screenshotcount-" + str(conf.PLATFORM) + ".csv", index_col=0)
    ...
    csv_filename = ("./data/stats-" + str(conf.PLATFORM) + ".csv")
...
def train_test_model(train_df, test_df, model, preprocessing_function, image_size, split, batch_size):
    ...
    # save history
    history_filename = "./results/history/vg-history-platform" + str(conf.PLATFORM) + "-" + model.name + "-" + str(split) + ".npy"
...
if __name__ == "__main__":
    ...
    # create filenames
    log_filename = "vg-platform" + str(conf.PLATFORM) + '-' + hostname + ".log"
    csv_filename = "vg-platform" + str(conf.PLATFORM) + '-' + hostname + ".csv"
    ...
    df, total_games, total_screenshots, selected_games, selected_screenshots = load_data()    
    ...
    for model_type in model_type_list:                  
        ...
        for index, [train, test] in enumerate(kf.split(df,df.category)):    
            ...
            with open(csv_filename,"a+") as f_csv:
                f_csv.write("%.4f" % acc)
                if index+1<kfold_n_splits: f_csv.write(", ")
                else: f_csv.write("\n")          
